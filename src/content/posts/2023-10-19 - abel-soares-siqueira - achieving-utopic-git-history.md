...
In a previous blog post, we defined [The Utopic Git History](https://blog.esciencecenter.nl/the-utopic-git-history-d44b81c09593) (UGH). To refresh our memory, a simple definition of a UGH is
...
The target audience for this post is Git users who want to learn intermediate and advanced commands that splice, merge, and fix commits. If you need a refresher on some of the concepts used, please take a look at the [animations in my previous blog post](https://blog.esciencecenter.nl/the-utopic-git-history-d44b81c09593).
...
![Meme. Left image shows dirty hands with a caption “My hands look like this” and right image shows git branches being transformed into linear git history with a caption “So my git history can look like this.”](/assets/1_w6RS2UcGCMtyvWSI2Eg7Ow-c5a1c936.gif)

Getting your hands dirty. Left photo by Jeremy Bishop on Unsplash. Modifications by me.
...
+ Something else
...
(1/n) Stage this hunk [y,n,q,a,d,s,e,?]?
```

Pressing `?` will show the help. There are four commands that matter a lot to us. The first two are y and n, for yes and no. If we say yes, the “hunk” (a piece of code shown in this diff) will be staged. Remember, when we `git commit` (no `-a` flag), the staged part of the code will be added to the commit. If we say no, then it just moves on to the next hunk.
...
In the command above, `code --wait` will open VSCode. Change it to your preference. Also, you can configure it globally using `--global` instead of `--local`.
...
Now, our task A1 is to rename y to p, and task A2 is to change all additions to subtractions.
...
The diff, which we will see when we enter edit mode, is the following:
...
Clearly, tasks A1 and A2 are mixed and can’t be split. So instead, we manually change the diff. Since we want only A1 applied, we change the diff to
...
This git history is not great. We still only have two tasks (A1 and A2), but 4 commits. What we want to do is redistribute the work done in these 4 commits into 2 commits. For that, we use `git rebase -i`, i.e., interactive rebase.
...
This will produce a graph of the commits, one per line, which should be easy to read. It should be something like

```hs
* hash04: (feat_br) Fix linting issue (add spaces around operators)
* hash03: Rename y to p and change + to - in function bar also
* hash02: Change + to -
* hash01: Rename y to p
* hash00: (main) Older stuff
```

This assumes that we branched out of `main` into a branch called `feat_br`. Now, to start rebasing, we can issue

```hs
git rebase -i main feat_br
```

In other words, it is just the normal rebase command, but now we add the `-i` flag to have more options on how to rebase.
...
pick hash04 Fix linting issue (add spaces around operators)
```

Notice that the commit at `main` is not included, as expected.
...
This will drop us in the terminal with
...
Now, we will use `reset` to undo this single commit and add only the change for task A1.
...
Make the first commit diff look like
...
Now we can run `git rebase --continue` and `git log --graph --oneline` again to obtain:
...
Notice that the hashes changed only after we effectively rebased.
...
pick newh03 Fix linting issue (add spaces around operators)
```
...
fixup newh03 Fix linting issue (add spaces around operators)
```

This will apply the first commit and then fix it, leaving only a single commit. Then, it will apply the second commit and fix it twice. None of these changes led to conflicts.
...
In other words, we have achieved UGH for this branch.
...
```hs
git branch feature_taskA more01
```

In other words, we are creating a branch called `feature_taskA` at the commit with hash `more01`. Push this branch

```hs
git push -u origin feature_taskA
```

It can be reviewed and rebased.
...
```hs
git rebase --onto=main feature_taskA feat_br
```

Here, we are saying “Take all commits after `feature_taskA` (exclusive) that are part of branch `feat_br`, and reapply them on top of the branch `main`.” We can use a hash instead of `main`, if necessary.
...
If the merge had conflicts, it means that we made commits to `main`, and we shouldn’t. We fix this by creating the branch now and resetting main:
...
Now, after working on branch `featureX`, we check for updates on `main ` again, using `fetch` and `merge --ff-only`. If there were any changes to main, we rebase after pushing:

```hs
git rebase main featureX
```

We might have conflicts, but since our branch is atomic, they should be few and far between.
...
Now, let’s say that we had to make changes because of the review. After making our changes, we use interactive rebase to make sure that the commits are atomic again. Our branch will not be in sync with the remote branch anymore, because the commits have changed (either in content or in hash).

That means that we now have to force-push our branch:

```hs
git push --force featureX
```

This should be enough to update the pull request.
