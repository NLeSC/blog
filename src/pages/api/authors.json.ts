...
export async function GET() {
  ...
  const data = [...byAuthor.entries()]
    .map(([name, authorPosts]) => {
      const sorted = authorPosts.sort((a, b) => comparePostsByDateDesc(a, b));
      const latestPost = sorted.at(0);
      return {
        name,
        ...
        latestPostDate: latestPost ? postDate(latestPost).toISOString() : null,
